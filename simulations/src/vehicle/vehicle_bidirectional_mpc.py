...
class VehicleBidirectionalMpc(Vehicle):
    ...
    def __init__(self, order, num_followers, init_speed, init_travel_distance, init_position, init_distance, vehicle_specs):
        ...
        self.D_continuous = np.array([[0, 0, 0, 0, 0]])
        ...
        dt = 0.01
        self.sys = ct.ss(self.A_discrete, self.B_discrete, self.C_discrete, self.D_discrete, dt=dt)

        prediction_horizon = 10  # Adjust according to your requirements
        control_horizon = 2  # Adjust according to your requirements
        #self.mpc_controller = ct.mpc(self.sys, dt=dt, prediction_horizon=prediction_horizon, control_horizon=control_horizon)
